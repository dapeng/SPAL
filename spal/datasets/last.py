...
class LaST(BaseDataset):
    ...
    def __init__(self, root='data', verbose=True, **kwargs):
        ...
        self.images_dir = osp.join(root, self.images_dir)
        self.train_dir = osp.join(self.images_dir, 'train')
        self.query_dir = osp.join(self.images_dir, 'val', 'query')
        self.gallery_dir = osp.join(self.images_dir, 'val', 'gallery')
        self.query_test_dir = osp.join(self.images_dir, 'test', 'query')
        self.gallery_test_dir = osp.join(self.images_dir, 'test', 'gallery')
        ...
        if verbose:
            print("=> LaST loaded")
            self.print_dataset_statistics_movie(self.train, self.query, self.gallery, self.query_test,
                                                self.gallery_test)

        self.num_train_pids, self.num_train_imgs, self.num_train_cams = self.get_imagedata_info(self.train)
        self.num_query_pids, self.num_query_imgs, self.num_query_cams = self.get_imagedata_info(self.query)
        self.num_gallery_pids, self.num_gallery_imgs, self.num_gallery_cams = self.get_imagedata_info(self.gallery)
        self.num_query_test_pids, self.num_query_test_imgs, self.num_query_test_cams = self.get_imagedata_info(
            self.query_test)
        self.num_gallery_test_pids, self.num_gallery_test_imgs, self.num_gallery_test_cams = self.get_imagedata_info(
            self.gallery_test)
